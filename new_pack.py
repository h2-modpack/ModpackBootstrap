...
The shell repo is created as a sibling of the ModpackBootstrap folder:
  ../speedrun-modpack/
...
import re
import tomllib
from bootstrap_common import rmtree, fill, write, run
...
def main():
  ...
  Side effects
    ...
    submodules, and installs ModpackTools as the ongoing pack toolbelt.
  ---------------------------------------------""")
    ...
    templates_dir = os.path.join(BOOTSTRAP_DIR, "templates", "coordinator")
    ...
    # -------------------------------------------------------------------------
    shell_templates_dir = os.path.join(BOOTSTRAP_DIR, "templates", "shell")
    ...
    # -------------------------------------------------------------------------
    # ModpackTools submodule
    # -------------------------------------------------------------------------
    print("\n>>> Adding ModpackTools submodule...")
    run(["git", "submodule", "add", "--branch", "main", TOOLS_URL, "ModpackTools"], cwd=output)
